...
function daily_financial_report() {
    ?>
    <div class="wrap">
        <h1>Daily Financial Report</h1><br>
            <style>
            .payment-label-bold {
                font-weight: bold;
            }
...
$payments = $wpdb->get_results(
    $wpdb->prepare("
        SELECT student_id, payment_method, last_paid_amount, paid_months, class_name,instructor_name
        FROM $table
        WHERE last_paid_date BETWEEN %s AND %s
    ", $start_date, $end_date),
    ARRAY_A
...
$summaries = [
    'cash' => '<table style="width:100%; border-collapse: collapse; border: 1px solid #000;">
        <tr>
            ...
            <th style="border: 1px solid #000; padding: 6px;">Instructor Name</th>
...
foreach ($payments as $payment) {
    ...
    $instructor_name = esc_html($payment['instructor_name']);
    ...
    if (isset($summaries[$method])) {
        $summaries[$method] .= "<tr>
            ...
            <td style='border: 1px solid #000; padding: 6px;'>{$instructor_name}</td>
